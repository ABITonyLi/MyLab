...
WebUI.click(findTestObject('Object Repository/Page_ABI One Process Center/NeedToDoTask/btn_internalconsult-' + (loc + 1).toString()))

WebUI.switchToWindowTitle('批量内部磋商')

WebUI.verifyElementText(findTestObject('Page_ABI One Process Center/NeedToDoTask/Batchinternalconsult_page/span_batchcontractcount'), 
    '共计 2 条合同条目，目前已展示 2 条')
'后置处理-内部磋商退回起草'
WebUI.callTestCase(findTestCase('合同签署流程(批量)/ProcessSteps/内部磋商退回起草'), [:], FailureHandling.STOP_ON_FAILURE)
'后置处理-经办人取消实例'
WebUI.callTestCase(findTestCase('合同签署流程(批量)/ProcessSteps/经办人取消实例'), [('applycontent') : TCcallback.get('contractname')
